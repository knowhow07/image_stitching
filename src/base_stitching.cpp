...
void BaseImageStitcher::setMinMatches(int matches) { min_matches = matches; }
void BaseImageStitcher::setRatioThreshold(float ratio) { ratio_thresh = ratio; }
    
// Main stitching function
cv::Mat BaseImageStitcher::stitch(const cv::Mat& img1, const cv::Mat& img2) {
...
cv::Mat BaseImageStitcher::stitchMultiple(const std::vector<cv::Mat>& images) {
    if (images.size() < 2) {
